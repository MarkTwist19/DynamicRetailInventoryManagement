...
with st.sidebar:
    ...
    if st.button("🔄 Generate Test Data"):
        with st.spinner("Generating test data..."):
            ...
            dg.generate_initial_stock(conn)
            
            # Generate sales data
            dg.generate_sample_sales(conn, days=30)
...
try:
    # Query to analyze stock situation
    query = """
    ...
    if df.empty:
            ...
            for store_id in online_stores['store_id'].unique():
                ...
                with st.expander(f"**{store_name}** | Needs: {len(needs)} SKUs | Excess: {len(excess)} SKUs", expanded=True):
                    ...
                    with tab1:
                        if not needs.empty:
                            st.dataframe(
                                needs[['sku', 'style_name', 'size', 'current_stock', 'units_sold', 'days_sold']]
                    ...
                    with tab2:
                        if not excess.empty:
                            st.dataframe(
                                excess[['sku', 'style_name', 'size', 'current_stock', 'units_sold', 'days_sold']]
                        ...
                        if not excess.empty:
                            ...
                            st.dataframe(
                                excess_display.head(10).rename(columns={
                                    ...
                                    'size': 'Size',
                                    'current_stock': 'Current',
                                    'units_sold': f'Sold ({days_lookdown}d)'
                                }),
                                use_container_width=True
                            )
                        else:
        ...
        SELECT 
            ...
            needs.units_sold as needs_sales,
            excess.store_name as has_stock_store,
            excess.current_stock as has_qty,
            excess.units_sold as has_sales,
            MIN(? - needs.current_stock, excess.current_stock - ?) as transfer_qty
        FROM (
            SELECT * FROM stock_info 
            WHERE units_sold > 0 AND current_stock <= ?
        ) needs
        JOIN (
            SELECT * FROM stock_info 
            WHERE units_sold = 0 AND current_stock >= ?
        ) excess ON needs.sku = excess.sku
        WHERE needs.store_id != excess.store_id
        ORDER BY needs.units_sold DESC, transfer_qty DESC
        LIMIT 20
        """
                    ...
                    with cols[3]:
                        if st.button(f"Create Transfer", key=f"transfer_{idx}"):
                            ...
                            cursor.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?)
                            """, (
                                row['has_stock_store'].split()[0],  # Extract store ID
                                row['needs_store'].split()[0],
                                row['sku'],
                                int(row['transfer_qty']),
                                'high' if row['needs_sales'] > 3 else 'medium',
                                f"Store selling but low stock (sold {row['needs_sales']} units)"
                            ))
                            conn.commit()
    ...
    st.info("Please generate test data first using the button in the sidebar.")
